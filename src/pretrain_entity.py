...
if __name__ == "__main__":
    ...
    elif cfg["dataset"]["class"] == "WikiTopicsMeta":
        filtered_data = [
            Data(
                edge_index=torch.cat([trg.target_edge_index, valg.target_edge_index], dim=1), 
                edge_type=torch.cat([trg.target_edge_type, valg.target_edge_type]),
                num_nodes=trg.num_nodes).to(device)
            for trg, valg, testg in zip(train_data, valid_data, test_data)
        ]
    ...
    if cfg["dataset"]["class"] == "WikiTopicsMeta":
        filtered_data = [
            Data(
                edge_index=torch.cat([testg.edge_index, testg.target_edge_index], dim=1), 
                edge_type=torch.cat([testg.edge_type, testg.target_edge_type]),
                num_nodes=testg.num_nodes).to(device)
            for trg, valg, testg in zip(train_data, valid_data, test_data)
        ]
